...
class UserProfileManager(models.Manager):
  ...
  def toggle_follow(self, user, to_toggle_user): # User.objects.toggle_follow(user=request.user, to_toggle_user)
  ...
  def is_following(self, user, followed_by_user): # User.objects.is_following(user, followed_by_user)
    user_profile, created = UserProfile.objects.get_or_create(user=user)
    if created:
      return False
  ...
  def recommended(self, user, limit_to=10): # User.objects.recommended(user=request.user)
